...
/** Proofreads a bio or prompt answer in place — fixes typos/grammar, keeps
 *  voice. Never overwrites automatically: shows a preview the user accepts
 *  or dismisses, and accepting stays undoable until they navigate away. */
  ...
  const handleRefine = async () => {
    ...
    try {
      const res = await fetch("/api/profile/refine-text", {
        ...
        body: JSON.stringify({ text: value, fieldType, promptContext }),
        credentials: "include",
      });
      if (res.status === 429) {
